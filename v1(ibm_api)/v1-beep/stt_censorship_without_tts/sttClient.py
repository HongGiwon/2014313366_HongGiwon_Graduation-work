            ...
            # if in streaming
                ...
                # regular hypothesis
                    ...
                    if bFinal:
                        ...
                        for t_word in res['alternatives'][0]['timestamps']:
                            if t_word[0] in b_dic:
                                b_word_arr.append(t_word[0])
                                b_time_arr.append(t_word[1])
                                b_time_arr.append(t_word[2])

                        censor_audio(audio_list, b_time_arr, b_word_arr)
...
def tts_word(word):
    text_to_speech = TextToSpeechV1(username='c124baa0-d10b-4182-821e-aa1977a43681',password='wX3oQ0jC8jP3tA4oR0rT5vK3bX7kC8kH6jT3iE5pY4hP7yH7aG',x_watson_learning_opt_out=True)

    print(json.dumps(text_to_speech.voices(), indent=2))
    s_word= AudioSegment.from_wav(StringIO(text_to_speech.synthesize(word, accept='audio/wav',voice="en-US_AllisonVoice")))
...
def censor_audio(audiofile, c_time_list, c_word_arr):
    audioSound = AudioSegment.from_mp3(audiofile[0])
    BeepSound = AudioSegment.from_mp3('beep/censor-beep-2.mp3')

    intervals = list(map(float,c_time_list))
    censored_result = audioSound
    interval_change = 0

    for index in range(0, len(c_word_arr)) :

        #TTS
        #c_word_sound = tts_word(c_word_arr[index])
        #i1 = intervals[index*2]*1000 + interval_change
        #i2 = intervals[index*2+1]*1000 + interval_change
        #s1 = censored_result[:i1]
        #s2 = censored_result[i2:]
        #censored_result = s1 + c_word_sound + s2
        #interval_change = interval_change + len(c_word_sound) - (i2 - i1)
        interval_dur = (intervals[index*2+1] - intervals[index*2]) * 1000
        beep_count = int((interval_dur / 500))
        remain = interval_dur - (beep_count * 500)
        sound_beep = BeepSound * beep_count + BeepSound[:remain]
        s1 = censored_result[:intervals[index*2]*1000]
        s2 = censored_result[intervals[index*2+1]*1000:]
        censored_result = s1 + sound_beep + s2

    censored_result.export("./output/0.mp3", format="mp3")
